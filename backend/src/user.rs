...
use matrix_sdk::{
    Client, ClientBuildError, Error, config::SyncSettings, event_handler::EventHandlerHandle,
...
impl User {
    ...
    pub async fn create_room_with(&self, user_id: &UserId) -> Result<Room, Error> {
    ...
    pub fn enable_sync(&self) -> JoinHandle<Result<Infallible, Error>> {
        tokio::spawn({
            ...
            async move {
                ...
                Err(Error::UnknownError("Synchronisation was terminated".into()))
            }
        })
    }
    ...
    /// # Errors
    ///
    /// Returns a [`ClientBuildError`] if the client failed to connect to the homeserver.
    ...
    where
        F: Fn(SyncRoomMessageEvent) + Clone + Sync + Send + 'static,
    {
        self.client
            .add_event_handler(async move |event: SyncRoomMessageEvent| handler(event))
    }
